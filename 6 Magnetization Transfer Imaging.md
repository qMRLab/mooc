...
Magnetization Transfer (MT) imaging encompasses a family of MRI techniques that exploit the interactions between different proton populations in biological tissues to generate unique contrast mechanisms. At its core, MT imaging examines the exchange of magnetization between highly mobile water protons (the "free pool") and protons associated with macromolecules and semi-solid structures (the "restricted pool"). The restricted pool protons have ultrashort transverse relaxation times (_T_{sub}`2` ~ 10-50 μs), making them undetectable with conventional MRI techniques. This ultrashort _T_{sub}`2` leads to substantial frequency broadening of the restricted pool's absorption lineshape, which MT techniques leverage by applying off-resonance radiofrequency pulses that selectively saturate macromolecular protons while minimally affecting the free water pool {cite:p}`Wolff1989-ag;Henkelman1993-lt`. 

The pervasive nature of MT effects means that virtually all MRI sequences are impacted by MT interactions. Even on-resonance pulses can pump the restricted pool, which subsequently exchanges magnetization with the water pool, particularly for sequences that use large flip angles such as inversion pulses and spin-echo trains. Off-resonance effects are especially pronounced in multi-slice acquisitions, where RF pulses intended for neighboring slices can inadvertently saturate macromolecular protons in the slice of interest {cite:p}`Santyr1993-ol`. These MT effects can significantly bias quantitative MRI measurements, contributing substantially to the challenges of standardizing quantitative MRI across different platforms and protocols. Ignoring MT effects has been shown to bias _T_{sub}`1` mapping {cite:p}`Asslander2025-zz;Asslander2025-yz` and _T_{sub}`2` mapping, where the extensive use of radiofrequency pulses in multi-echo sequences can lead to underestimation of relaxation times. By deliberately manipulating these exchange processes through specialized RF preparation pulses, MT techniques can generate contrast that reflects macromolecular density and organization {cite:p}`Sled2018-zr`, providing valuable insights into tissue microstructure that complement conventional relaxation-based contrasts.

By deliberately manipulating these exchange processes through specialized RF preparation pulses, MT techniques can generate contrast that reflects macromolecular density and organization {cite:p}`Sled2018-zr`. This capability has enabled important applications across clinical and research domains, particularly for assessing myelin content in neurological disorders such as multiple sclerosis {cite:p}`Schmierer2004-yr;Schmierer2007-mb`, and has also shown utility in studying Alzheimer's disease {cite:p}`Fornari2012-va` and psychiatric disorders {cite:p}`Chen2015-ch`.

This chapter explores three principal methodologies for magnetization transfer imaging, each offering different levels of quantitative rigor and practical implementation. Quantitative Magnetization Transfer (qMT) represents the most comprehensive approach, employing multi-parameter fitting of the Bloch-McConnell equations to extract fundamental tissue properties such as pool size ratio, exchange rates, and relaxation times {cite:p}`Sled2001-fz;Sled2018-zr`. While providing the most detailed characterization of tissue microstructure, qMT requires extensive acquisition protocols and complex fitting procedures, making it challenging for routine clinical use. In contrast, Magnetization Transfer Ratio (MTR) offers a simplified, semi-quantitative approach that calculates the normalized signal difference between images acquired with and without MT preparation {cite:p}`Wolff1989-ag`. Despite its widespread clinical adoption, particularly in multiple sclerosis research, MTR suffers from dependencies on sequence parameters and tissue relaxation properties that limit its quantitative accuracy.

More recently, Magnetization Transfer Saturation (MTsat) has emerged as an intermediate approach that combines the practicality of MTR with improved quantitative characteristics {cite:p}`Helms2008-wf`. By incorporating additional measurements and analytical corrections, MTsat provides better separation of MT effects from confounding factors like tissue _T_{sub}`1` variations, while maintaining clinically feasible acquisition times. Throughout this chapter, we will examine the theoretical foundations, practical implementations, and relative strengths of each MT imaging approach, providing readers with comprehensive understanding of how these techniques can be applied to study tissue microstructure across various research and clinical contexts.
