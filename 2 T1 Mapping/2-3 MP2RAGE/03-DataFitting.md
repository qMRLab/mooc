...
To produce _T_{sub}`1` maps with good accuracy and precision using dictionary-based interpolation methods, it is important that the signal curves are unique for each parameter value. MP2RAGE can produce good _T_{sub}`1` maps by using a dictionary with only dimensions (_T_{sub}`1`, _S_{sub}`MP2RAGE`), since _S_{sub}`MP2RAGE` is unique for each _T_{sub}`1` value for a given protocol  (Marques et al. 2010). However, as was noted above, _S_{sub}`MP2RAGE` is also sensitive to _B_{sub}`1` because of _θ_{sub}`1` and _θ_{sub}`2` in  Equations [2.13](#vfaEq3), [2.13](#vfaEq4), [2.15](#vfaEq5), and [2.13](#vfaEq6). The  _B_{sub}`1`-sensitivity can be reduced substantially with careful MP2RAGE protocol optimization (Marques et al. 2010), and further improved by including _B_{sub}`1` as one of the dictionary dimensions [_T_{sub}`1`, _B_{sub}`1`, _S_{sub}`MP2RAGE`] ([](#mp2rageplot1)).  This requires an additional acquisition of a _B_{sub}`1` map (Marques & Gruetter 2013), which lengthens the scan time. 
