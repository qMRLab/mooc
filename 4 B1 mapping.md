...
Transmit radiofrequency field mapping (_B_{sub}`1`{sup}`+`, or _B_{sub}`1` for short) is an essential component of quantitative MRI. Accurate knowledge of this field distribution is crucial for multiple applications, including the calibration of quantitative MRI techniques that are sensitive to flip angle inaccuracies, such as variable flip angle _T_{sub}`1` mapping {cite:p}`Deoni2007-se;Boudreau2017-ik` and quantitative magnetization transfer imaging {cite:p}`Ropele2005-pm;Boudreau2018-kv`. Unlike the receive field (_B_{sub}`1`{sup}`-`), which appears as a simple multiplicative factor that can often be corrected for in post-processing using intensity normalization techniques {cite:p}`Sled1998-fr`, the transmit field has a nonlinear impact on MR signal evolution. For example, in spoiled gradient echo sequences, the signal depends on the actual flip angle through complex trigonometric functions in the steady-state signal equation {cite:p}`Christensen1974,Fram1987,Gupta1977`, making it impossible to correct images without detailed knowledge of both the actual flip angles and the specific signal model. _B_{sub}`1` maps also play important roles in specific absorption rate calculations {cite:p}`Ibrahim2001-yj`, RF coil quality control {cite:p}`Yarnykh2007-bv`, and the study of tissue electrical properties {cite:p}`Sled1998-pz;Katscher2009-lk`.

This chapter explores several principal methodologies for _B_{sub}`1` mapping, each with distinct advantages and practical considerations. The double angle method represents a straightforward magnitude-based approach that acquires images at two different excitation flip angles and computes _B_{sub}`1` from their signal ratio {cite:p}`Insko1993-ik;Stollberger1996-kh`. While conceptually simple, this technique typically requires long repetition times to minimize _T_{sub}`1` dependence, which can limit its time efficiency. In contrast, actual flip-angle imaging (AFI) employs a pulsed steady-state acquisition with two different repetition times, enabling rapid 3D _B_{sub}`1` mapping with excellent anatomical coverage in clinically feasible times {cite:p}`Yarnykh2007-bv`. More recently, phase-based methods like the Bloch-Siegert shift technique have emerged that encode _B_{sub}`1` information directly into phase signals, offering alternative acquisition strategies {cite:p}`Sacolick2010-ga`.

Regardless of the acquisition method, measured _B_{sub}`1` maps often require post-processing to address noise, artifacts, and other non-physical variations. Filtering techniques play an important role in producing field maps that better represent the underlying smoothly varying electromagnetic fields while minimizing error propagation to subsequent quantitative MRI applications. Throughout this chapter, we will examine the theoretical foundations, practical implementations, and relative strengths of each _B_{sub}`1` mapping approach, providing readers with comprehensive understanding of modern RF field mapping techniques.
